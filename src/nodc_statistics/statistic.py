...
from nodc_statistics import calculate_parameter
...
class DataHandler:
    ...
    def _add_parameters(self):
        ...
        self.data.loc[:, "salt"] = self.data.copy().apply(
            lambda row: calculate_parameter.get_prio_par(row.SALT_CTD, row.SALT_BTL, row.Q_SALT_CTD, row.Q_SALT_BTL), axis=1
        )
        self.data.loc[:, "temp"] = self.data.copy().apply(
            lambda row: calculate_parameter.get_prio_par(row.TEMP_CTD, row.TEMP_BTL, row.Q_TEMP_CTD, row.Q_TEMP_BTL), axis=1
        )
        calculate_parameter.map_to_standard_depth(data=self.data, standard_depths=self.settings['standard_depths'])
...
class CalculateStatistics:
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        with SETTING_FILE['settings'].open('r', encoding='utf-8') as file:
            self.settings = json.load(file)
        self.area_tags_df = pd.read_csv(open(AREA_TAG_FILE, encoding='utf-8'),
                            sep="\t")
        self._set_up_areas()
    ...
    def _set_up_areas(self):
        self.data['pos_string'] = self.data.apply(lambda row: '_'.join([str(row.LONGI_DD), str(row.LATIT_DD)]), axis=1)
    ...
    def _map_areas_to_pos_str(self):
        ...
        self.data = pd.merge(self.data, area_tags_filtered, on='pos_string', how='left', suffixes=('_old', ''))
    ...
    def profile_statistics(self):
        ...
        # Skapa dictionary för aggregering
        agg_dict = {param: list(agg_funcs.values()) for param in self.settings['statistic_parameters']}
        ...
        names = {col[1]: agg[0] for col, agg in zip(grouped.columns[0:len(agg_funcs)], agg_funcs.items())}
        new_columns = []
    ...
    def _save_statistic_files(self, data, column_name='area_tag', file_format='csv'):
        ...
        for area_tag, group in grouped:
            ...
            group.to_csv(Path(__file__).parent / "data/statistics" / file_name, **SAVE_KWARGS)
