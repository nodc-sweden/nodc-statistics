...
def test_get_profile_statistics_for_parameter_returns_dictionary():
    ...
    statistics_object = statistics.get_profile_statistics_for_parameter_and_position(
...
@pytest.mark.parametrize(
    ...
    (
        ("TEMP", 58.3050, 10.759, datetime.datetime(2024, 5, 16), (-2, 30)),
        ("DOXY", 58.3050, 10.759, datetime.datetime(2024, 5, 16), (2, 11)),
    ),
)
def test_mean_values_in_expected_range(
    ...
    assert all(lower_bound < value < upper_bound for value in statistics_object["mean"])


@pytest.mark.parametrize(
    "given_parameter, given_latitude, given_longitude, given_datetime",
    (
        ("TEMP", 58.3050, 10.759, datetime.datetime(2024, 5, 16)),
        ("DOXY", 58.3050, 10.759, datetime.datetime(2024, 5, 16)),
    ),
)
...
def test_positions_in_different_sea_basins_have_different_statistics(given_parameter):
    ...
    given_point_in_time = datetime.datetime(2024, 5, 16, 15, 2)

    skagerrak_statistics = statistics.get_profile_statistics_for_parameter_and_position(
        given_parameter, skagerrak_longitude, skagerrak_latitude, given_point_in_time
    )

    kattegat_statistics = statistics.get_profile_statistics_for_parameter_and_position(
...
def test_mean_surface_temperature_is_lower_in_february_than_in_august():
    february_statistics = statistics.get_profile_statistics_for_parameter_and_position(
        "TEMP", 11.619563447195599, 57.63218414832167, datetime.datetime(2024, 2, 1)
    )

    august_statistics = statistics.get_profile_statistics_for_parameter_and_position(
        "TEMP", 11.619563447195599, 57.63218414832167, datetime.datetime(2023, 8, 1)
    )
